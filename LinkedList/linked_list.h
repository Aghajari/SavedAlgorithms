 ...
 *
 * @return 0 If the node does not match the target, otherwise the distance from the node.
 * (Negative number indicates before and positive after node)
 */

/* A Finder function to find the node by index, typeof target must be (int *) */
int linked_list_finder_index(struct LinkedListNode *node, int index, const void *target);

/* A Finder function to find the node by data, typeof target must be (void **) */
int linked_list_finder_data(struct LinkedListNode *node, int index, const void *target);

/* A Finder function to find the last node in list, target can be NULL */
...
/* Returns parent of the specified node in the list (Or NULL if it doesn't exist). */
...
/*
* Replaces the node's data at the specified position in this list with the specified data.
...
/*
* Removes and frees the specified node in the list.
* @return 1 If the operation was performed, 0 otherwise
*/
...
/*
 * Specifies whether to free the data of the node by removing it in linked list.
 */
